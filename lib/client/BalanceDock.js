...
function UsageBars({ days, t }) {
    ...
    return (_jsxs("svg", { width: width, height: height, role: "img", "aria-label": t('usage.title'), children: [ticks.map((tick) => {
                ...
                return (_jsxs("g", { onMouseEnter: () => setHovered(index), onMouseLeave: () => setHovered(null), children: [_jsx("rect", { x: x, y: y, width: barW, height: barH, rx: 2, fill: active ? 'var(--dsw-accent-strong, #7aa8ff)' : 'var(--dsw-accent, #4c8dff)' }), _jsx("text", { x: x + barW / 2, y: height - 5, textAnchor: "middle", fontSize: 8, fill: "currentColor", children: day.date.slice(5) }), active && _jsx(BarTooltip, { day: day, barX: x + barW / 2, width: width, t: t })] }, day.date));
            })] }));
}
/** The hover card pinned to the top of the chart, describing one bar. */
function BarTooltip({ day, barX, width, t, }) {
    const tipW = 118;
    const tipH = 32;
    const x = Math.min(Math.max(barX - tipW / 2, 0), width - tipW);
    const y = 2;
    return (_jsxs("g", { children: [_jsx("rect", { x: x, y: y, width: tipW, height: tipH, rx: 4, fill: "var(--dsw-surface, #1f2430)", stroke: "rgba(128,128,128,0.45)" }), _jsx("text", { x: x + 6, y: y + 12, fontSize: 9, fill: "currentColor", children: t('usage.bar', { date: day.date, cost: formatCny(day.cost) }) }), _jsx("text", { x: x + 6, y: y + 24, fontSize: 9, fill: "currentColor", children: t('usage.barDetail', {
