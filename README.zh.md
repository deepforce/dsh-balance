...
- **每日消耗图** — 悬停 `近{days}天 ≈ ¥…` 时，一张带标题的柱状图会像漫画对话气泡一样从摘要**向上浮出**：纵轴为金额刻度（¥），横轴每根柱子标注日期，hover 柱子显示当天详情（日期、金额、请求数、token 明细），移出即收起。数据来自宿主端 `/dsh-usage` 路由：遍历已持久化的会话，折叠 `assistant/message` 的用量，并按每次请求发生时所在的时段单独计价。
