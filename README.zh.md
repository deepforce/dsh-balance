...
- **Web GUI 底部余额条** — 对话页统计条区域（缓存命中率下方）显示简洁的 `余额: ¥…`，hover 显示充值/赠送详情，附带 **充值** 链接（跳转 DeepSeek 平台充值页）、手动刷新按钮，以及估算的 **本会话 ≈ ¥…** 花费（hover 显示 token 分桶 × 单价）。浏览器只请求宿主端的 `/dsh-balance` 路由；API key 始终留在宿主端。
